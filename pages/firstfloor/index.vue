<template>
  <div class="first__floor-wrapper">
    <b-container>
      <div class="floor__wrapper">
        <h1  class="text-center">First Floor</h1>
        <div class="floor__header-part">
          <nuxt-link to="" class="floor__link">
            <img src="../../assets/images/1floor/1.jpg" alt="">
          </nuxt-link>
        </div>
        <div class="floor__top-part">
          <nuxt-link :to="localePath(`/firstfloor/1`)" class="floor__link">
            <img src="../../assets/images/1floor/2.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/2`)" class="floor__link">
            <img src="../../assets/images/1floor/3.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/3`)" class="floor__link">
            <img src="../../assets/images/1floor/4.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/4`)" class="floor__link">
            <img src="../../assets/images/1floor/5.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/5`)" class="floor__link">
            <img src="../../assets/images/1floor/6.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath('/firstfloor/6')" class="floor__link">
            <img src="../../assets/images/1floor/7.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath('/firstfloor/7')" class="floor__link">
            <img src="../../assets/images/1floor/8.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath('/firstfloor/8')" class="floor__link">
            <img src="../../assets/images/1floor/9.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath('/firstfloor/9')" class="floor__link">
            <img src="../../assets/images/1floor/10.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath('/firstfloor/10')" class="floor__link">
            <img src="../../assets/images/1floor/11.jpg" alt="" />
          </nuxt-link>
          <nuxt-link to="#" class="floor__link">
            <img src="../../assets/images/1floor/12.jpg" alt="" />
          </nuxt-link>
        </div>
        <div class="floor__bottom-part">
          <nuxt-link :to="localePath('#')" class="floor__link">
            <img src="../../assets/images/1floor/13.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/11`)" class="floor__link">
            <img src="../../assets/images/1floor/14.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/12`)" class="floor__link">
            <img src="../../assets/images/1floor/15.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/14`)" class="floor__link">
            <img src="../../assets/images/1floor/16.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/15`)" class="floor__link">
            <img src="../../assets/images/1floor/17.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/16`)" class="floor__link">
            <img src="../../assets/images/1floor/18.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath(`/firstfloor/17`)" class="floor__link">
            <img src="../../assets/images/1floor/19.jpg" alt="" />
          </nuxt-link>
          <nuxt-link :to="localePath('/firstfloor/18')" class="floor__link">
            <img src="../../assets/images/1floor/20.jpg" alt="" />
          </nuxt-link>
           <nuxt-link :to="localePath(`/firstfloor/19`)" class="floor__link">
            <img src="../../assets/images/1floor/21.jpg" alt="" />
          </nuxt-link>
           <nuxt-link to="#" class="floor__link">
            <img src="../../assets/images/1floor/22.jpg" alt="" />
          </nuxt-link>
        </div>
      </div>
    </b-container>
  </div>
</template>

<script>
export default {
  mounted() {
    console.log(this.$route.params.id)
  }
}
</script>

<style lang="scss" scoped></style>
